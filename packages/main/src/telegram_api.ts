...
export default class TelegramApi extends BotApi {
	...
	_executeCommand = async (update: TelegramUpdate, text: string, args: string[] = []) =>
	...
	executeInlineCommand = async (update: TelegramUpdate): Promise<Response> =>
		this._executeCommand(update, update.inline_query?.query ?? '').then(async (command_response) =>
			command_response
				? this._executeCommand(update, 'inline', update.inline_query?.query.trimStart().split(' ')).then(
	...
	answerInlineQuery = async (inline_query_id: number, results: TelegramInlineQueryResult[], cache_time = 0) =>
		fetch(
			log(
				addSearchParams(new URL(`${this.webhook.api.origin}${this.webhook.api.pathname}/answerInlineQuery`), {
	...
	editMessageText = async (chat_id: number, message_id: number, text: string): Promise<Response> =>
		fetch(
			log(
				addSearchParams(new URL(`${this.webhook.api.origin}${this.webhook.api.pathname}/editMessageText`), {
	...
	sendMessage = async (
		...
		fetch(
			log(
				addSearchParams(new URL(`${this.webhook.api.origin}${this.webhook.api.pathname}/sendMessage`), {
					chat_id: chat_id.toString(),
					text,
	...
	forwardMessage = async (chat_id: number, from_chat_id: number, disable_notification = false, message_id: number) =>
		fetch(
			log(
				addSearchParams(new URL(`${this.webhook.api.origin}${this.webhook.api.pathname}/sendMessage`), {
					chat_id: chat_id.toString(),
	...
	sendPhotoRaw = async (
		...
		return fetch(
			log(
				addSearchParams(new URL(`${this.webhook.api.origin}${this.webhook.api.pathname}/sendPhoto`), {
					chat_id: chat_id.toString(),
					caption,
					parse_mode,
					disable_notification: disable_notification.toString(),
	...
	sendVideo = async (
		...
		fetch(
			log(
				addSearchParams(new URL(`${this.webhook.api.origin}${this.webhook.api.pathname}/sendVideo`), {
					chat_id: chat_id.toString(),
	...
	sendAnimation = async (
		...
		fetch(
			log(
				addSearchParams(new URL(`${this.webhook.api.origin}${this.webhook.api.pathname}/sendAnimation`), {
					chat_id: chat_id.toString(),
	...
	sendLocation = async (
		...
		fetch(
			log(
				addSearchParams(new URL(`${this.webhook.api.origin}${this.webhook.api.pathname}/sendLocation`), {
					chat_id: chat_id.toString(),
	...
	sendPoll = async (
		...
		fetch(
			log(
				addSearchParams(new URL(`${this.webhook.api.origin}${this.webhook.api.pathname}/sendPoll`), {
					chat_id: chat_id.toString(),
					question,
	...
	sendDice = async (chat_id: number, emoji = '', disable_notification = false, reply_to_message_id = 0) =>
		fetch(
			log(
				addSearchParams(new URL(`${this.webhook.api.origin}${this.webhook.api.pathname}/sendDice`), {
					chat_id: chat_id.toString(),
					emoji,
					disable_notification: disable_notification.toString(),
					reply_to_message_id: reply_to_message_id.toString(),
	...
	getUserProfilePhotos = async (user_id: number, offset = 0, limit = 0): Promise<Response> =>
		fetch(
			log(
				addSearchParams(new URL(`${this.webhook.api.origin}${this.webhook.api.pathname}/getUserProfilePhotos`), {
