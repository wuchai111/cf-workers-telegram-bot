	...
	((outcome, message) =>
		update.inline_query
			...
			: self.sendMessage(
					...
					message(update.message?.from.username ?? '', update.message?.from.first_name ?? '', outcome),
				))(
		Math.floor(Math.random() * (parseInt(args[1]) || 6 - 1 + 1) + 1),
		(username: string, first_name: string, outcome: number) =>
			`${first_name ?? username} rolled a ${parseInt(args[1]) || 6} sided die. it landed on ${outcome}`,
	);
