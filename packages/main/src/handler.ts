...
export default class Handler {
	...
	getResponse = async (_request?: Request, _bot?: BotApi): Promise<Response> => {
		this.getAccessKeys(this.configs).then((access_keys) =>
			Object.keys(access_keys).forEach((key) =>
				log(
					`${access_keys[key].bot_name} ${((request_url) => `${request_url.origin}${request_url.pathname}`)(
	...
	postResponse = async (_request?: Request, _bot?: BotApi): Promise<Response> =>
		_bot?.webhook.token === ''
			...
			: _request
				? _request.json().then((update) => (_bot as BotApi).update(update as Update))
				: this.responses.default();
	...
	getAccessKeys = async (configs: Partial<Config>[]): Promise<Record<string, Config> | Record<string, never>> =>
		Promise.all(
